  ...
  selected_object_actions(e) {
    ...
    if (event_type == 'mousemove' && this.mouse_lock == 'selected_object_actions' && this.is_drag) {
      const allowNegativeDimensions =
        settings.data.render_function && ['line', 'arrow', 'gradient'].includes(settings.data.render_function[0]);
      ...
      if (drag_type == 'rotate') {
        ...
        if (
          (drag_type && (drag_type & (drag_type - 1)) !== 0 && settings.keep_ratio == true && is_ctrl == false) ||
