...
class Base_search_class {
  ...
  events() {
    ...
    document.addEventListener(
      ...
      (event) => {
        ...
        for (var i = 0; i < results.length; i++) {
          ...
          node.innerHTML +=
            "<div class='" + className + "' data-key='" + item.obj.key + "'>" + fuzzysort.highlight(item[0]) + '</div>';
        }
      },
      false,
    );
    ...
    document.addEventListener(
      ...
      function (e) {
        if (
          document.querySelector('#global_search_results') == null ||
  ...
  search() {
    ...
    var settings = {
      title: 'Search',
      params: [{ name: 'search', title: 'Search:', value: '' }],
