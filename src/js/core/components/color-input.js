  ...
  const on_click_color_input = (event) => {
    ...
    var settings = {
      ...
      on_finish() {
        set_value(
          $el,
          colorsDialog.COLOR + (colorsDialog.ALPHA < 255 ? colorsDialog.ALPHA.toString(16).padStart(2, '0') : ''),
        );
        $el.trigger('input');
        $el.trigger('change');
        colorsDialog = null;
  ...
  $.fn.uiColorInput = function (behavior, ...args) {
    ...
    for (let i = 0; i < this.length; i++) {
      ...
      if (Object.prototype.toString.call(behavior) !== '[object String]') {
        ...
        $(input).on('click', on_click_color_input).on('focus', on_focus_color_input).on('blur', on_blur_color_input);
