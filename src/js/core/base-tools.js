...
class Base_tools_class {
  ...
  events() {
    ...
    document.addEventListener(
      ...
      function (event) {
        _this.dragMove(event);
        if (event.target.id === 'canvas_minipaint' && !$('.scroll').has($(event.target)).length) event.preventDefault();
      },
      { passive: false },
    );
  ...
  set_mouse_info(event) {
    ...
    if (eventType === 'mousedown' || eventType === 'touchstart') {
      if (
        (event.target.id != 'canvas_minipaint' && event.target.id != 'main_wrapper') ||
  ...
  calc_snap_position(event, pos_x, pos_y, exclude_id) {
    ...
    if (typeof exclude_id != 'undefined') var snap_positions = this.get_snap_positions(exclude_id);
