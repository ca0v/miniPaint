...
class Bezier_Curve_class extends Base_tools_class {
  ...
  mousemove(e) {
    ...
    if (e.ctrlKey == true || e.metaKey) {
      ...
      if (Math.abs(width) > Math.abs(height)) mouse_y = click_y;
      else mouse_x = click_x;
    }
  ...
  render_overlay(ctx) {
    ...
    if (config.layer.type == this.name) {
      ...
      if (bezier.end.x != null && bezier.cp2.x != null) {
        this.Helper.draw_special_line(this.ctx, x + bezier.end.x, y + bezier.end.y, x + bezier.cp2.x, y + bezier.cp2.y);
        if (config.TOOL.name == 'select') {
  ...
  draw_bezier(ctx, x, y, data, lineWidth, color) {
    ...
    ctx.bezierCurveTo(x + data.cp1.x, y + data.cp1.y, x + data.cp2.x, y + data.cp2.y, x + data.end.x, y + data.end.y);
    ctx.stroke();
  }
