import { updateDialogSize, isLandscape } from '../dataworks-plus-extensions.js';
...
var template = `
	...
	<div class="dialog_content" data-id="dialog_content">
		<div data-id="preview_content" style="max-height:${maxHeight}px"></div>
		<div data-id="params_content" style="margin-top:20px"></div>
	</div>
	<div class="buttons">
		<button type="button" data-id="popup_save" style="display:none" class="button trn">Save</button>
...
class Dialog_class {
    ...
    set_events() {
        ...
        this.addEventListener(
            ...
            'mousedown',
            (event) => {
                if (event.target != this.el.querySelector('h2')) return;
    ...
    onChangeEvent(e) {
        ...
        if (this.last_params_hash == hash && this.onchange == false) {
            //nothing changed
            console.warn(`dataworks: nothing changed, but not returning`);
        }
        ...
        for (var i = 0; i < inputs.length; i++) {
            if (inputs[i].id.substr(0, 9) == 'pop_data_') {
                ...
                if (inputs[i].type == 'radio') {
                    ...
                    if (inputs[i].checked == true) response[key] = true;
    ...
    show_action() {
        ...
        if (!isLandscape()) {
            width = 200;
            this.width_mini = 200;
            this.height_mini = 250;
        } else {
            width = 300;
            this.width_mini = 300;
            this.height_mini = 240;
        }

        //preview area
        if (this.preview !== false) {
            // dataworks: customizations need to be re-applied to the css (near line 3817 in bundle_9771.js)
            html_preview_content += `
<div class="preview_container">
	<canvas class="preview_canvas_left" width="${this.width_mini}" height="${this.height_mini}" data-id="pop_pre"></canvas>
	<div class="canvas_preview_container">
		<canvas class="preview_canvas_post_back" width="${this.width_mini} height="${this.height_mini}" data-id="pop_post_back"></canvas>
		<canvas class="preview_canvas_post" width="${this.width_mini}" height="${this.height_mini}" data-id="pop_post"></canvas>
	</div>
</div>`;
        }
        ...
        const popupOk = this.el.querySelector('[data-id="popup_ok"]');
        ...
        {
            // dataworks
            document.querySelector('[data-id="popup_save"]').addEventListener('click', function (event) {
                $.isServerSave = true;
                _this.save();
            });
        }
    ...
    generateParamsHtml() {
        ...
        for (var i in this.parameters) {
            ...
            if (parameter.name != undefined) {
                if (parameter.values != undefined) {
                    if (parameter.values.length > 10 || parameter.type == 'select') {
                        ...
                        for (var j in parameter.values) {
                            ...
                            html +=
                                '<input type="radio" onchange="POP.onChangeEvent();" ' +
                                ch +
                                ' name="' +
                                parameter.name +
                                '" id="pop_data_' +
                    ...
                    if (parameter.range != undefined) {
                        ...
                        html +=
                            ...
                            '" step="' +
                            step +
                            '" oninput="document.getElementById(\'pv' +
                            i +
                            "').innerHTML = " +
                        ...
                        //color
                        html +=
                            '<td><input type="color" id="pop_data_' +
                            parameter.name +
                            '" value="' +
                            parameter.value +
                            '" onchange="POP.onChangeEvent();" /></td>';
                        ...
                        html +=
                            '<td class="checkbox"><input type="checkbox" id="pop_data_' +
                            parameter.name +
                            '" ' +
                            checked +
                            ' onclick="POP.onChangeEvent();" > <label class="trn" for="pop_data_' +
                            parameter.name +
                            '">Toggle</label></td>';
                    } else {
                        //input or textarea
                        ...
                        if (parameter.type == 'textarea') {
                            //textarea
                            html +=
                                '<td><textarea rows="10" id="pop_data_' +
                                parameter.name +
                            ...
                            html +=
                                ...
                                '" id="pop_data_' +
                                parameter.name +
                                '" onchange="POP.onChangeEvent();" value="' +
                                parameter.value +
                                '" placeholder="' +
                                parameter.placeholder +
                                '" ' +
                ...
                if (str.length < 40)
                    html +=
                        '<td colspan="2"><div class="trn" id="pop_data_' +
                        id_tmp +
                        '">' +
                        parameter.value +
                        '</div></td>';
                    ...
                    html +=
                        '<td class="long_text_value" colspan="2"><textarea disabled="disabled">' +
                        parameter.value +
                        '</textarea></td>';
            }
            html += '</tr>';
