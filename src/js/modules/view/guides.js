...
class View_guides_class {
    ...
    insert_handler(data) {
        ...
        var x = null;
        var y = null;
        if (type == 'Vertical') x = position;
        if (type == 'Horizontal') y = position;
    ...
    update_handler(data) {
        ...
        for (var i in data) {
            ...
            if (config.guides[key].x === null) config.guides[key].y = value;
            else config.guides[key].x = value;
        }
        ...
        for (var i = 0; i < config.guides.length; i++) {
            if (
                config.guides[i].x === 0 ||
                config.guides[i].y === 0 ||
                isNaN(config.guides[i].x) ||
                isNaN(config.guides[i].y)
            ) {
