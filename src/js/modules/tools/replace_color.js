  ...
  do_replace(data, params) {
    ...
    for (var i = 0; i < imgData.length; i += 4) {
      ...
      if (mode == 'Simple') {
        ...
        var diff =
          (Math.abs(imgData[i] - target_rgb.r) +
            Math.abs(imgData[i + 1] - target_rgb.g) +
            Math.abs(imgData[i + 2] - target_rgb.b)) /
          3;
        ...
        if (alpha < 255) imgData[i + 3] = alpha;
      } else {
        ...
        var diff =
          (Math.abs(normalized.r - target_normalized.r) +
            ...
            Math.abs(normalized.b - target_normalized.b)) /
          3;
        if (diff > power) continue;

        //change to new color with existing luminance
        var normalized_final = this.Helper.hslToRgb(replacement_hsl.h, replacement_hsl.s, hsl.l * replacement_hsl.l);
