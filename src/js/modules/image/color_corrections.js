...
class Image_colorCorrections_class {
    ...
    color_corrections() {
        ...
        var settings = {
            ...
            on_change: function (params, canvas_preview, w, h, canvas) {
                ...
                //non-destructive
                canvas_preview.filter =
                    'brightness(' + (1 + params.param_b / 100) + ')';
                canvas_preview.filter +=
                    ' contrast(' + (1 + params.param_c / 100) + ')';
                canvas_preview.filter +=
                    ' saturate(' + (1 + params.param_s / 100) + ')';
                canvas_preview.filter +=
                    ' hue-rotate(' + params.param_h + 'deg)';
    ...
    do_corrections(data, params) {
        //luminance
        if (params.param_l != 0) {
            var data = this.ImageFilters.HSLAdjustment(
                data,
                0,
                0,
                params.param_l,
