...
class Image_colorCorrections_class {
    ...
    save_changes(params) {
        ...
        if (params.param_b != 0) {
            ...
            var filter_id = null;
            app.State.do_action(new app.Actions.Add_layer_filter_action(null, 'brightness', parameters, filter_id));
        }
        if (params.param_c != 0) {
            var parameters = { value: params.param_c };
            var filter_id = null;
            app.State.do_action(new app.Actions.Add_layer_filter_action(null, 'contrast', parameters, filter_id));
        }
        if (params.param_s != 0) {
            var parameters = { value: params.param_s };
            var filter_id = null;
            app.State.do_action(new app.Actions.Add_layer_filter_action(null, 'saturate', parameters, filter_id));
        }
        if (params.param_h != 0) {
            var parameters = { value: params.param_h };
            var filter_id = null;
            app.State.do_action(new app.Actions.Add_layer_filter_action(null, 'hue-rotate', parameters, filter_id));
        }
    }
    ...
    do_corrections(data, params) {
        ...
        if (params.param_red != 0 || params.param_green != 0 || params.param_blue != 0) {
            var data = this.ImageFilters.ColorTransformFilter(
                data,
                1,
                1,
                1,
