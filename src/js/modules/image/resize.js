...
class Image_resize_class {
  ...
  async resize_layer(layer, params) {
    ...
    if (isNaN(width) || isNaN(height)) {
      ...
      if (isNaN(width)) width = Math.round(height * ratio);
      canvas_width = Math.round(canvas_height * canvas_ratio);
      if (isNaN(height)) height = Math.round(width / ratio);
        ...
        for (let span of line) {
          ...
          span.meta.stroke_size = parseFloat(
            (
              0.1 *
              Math.round(
                ((span.meta.stroke_size != null ? span.meta.stroke_size : textMetaDefaults.stroke_size) * xratio) / 0.1,
              )
            ).toFixed(1),
          );
